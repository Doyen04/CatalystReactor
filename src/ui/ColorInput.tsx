import {
    getDisplayTextFromFill,
    extractFillValue,
    imageValue,
    arrayBufferToDataUrl,
    getBackgroundStyleFromFillValue,
} from '@/util/getBackgroundFill'
import {
    Fill,
    FillType,
    GradientFill,
...
const ColorInput = forwardRef<HTMLDivElement, ColorInputProps>(
    ({ showTab = true, onChange, className, fill, ...props }, ref) => {
        ...
        if (fill.color.type === 'image' || fill.color.type === 'pattern') {
            backgroundStyle = getBackgroundStyleFromFillValue(
                fillValue.value,
                fill.color,
                imageUrl
            )
             ...
             transition-colors ${className}`)}
                >
                    <p
                        ...
                        className="font-semibold text-gray-700 h-4 w-4 rounded-xs"
                    ...
                    <Input
                        className="flex-1 min-w-0 rounded-none border-none hover:border-none hover:focus-within:border-none"
