...
const RadialGradientPicker: React.FC<RadialGradientPickerProps> = ({ value, onGradientChange, className }) => {
    ...
    //move
    const addStop = () => {
        const newOffset = gradient.stops.length > 0 ? Math.min(1, Math.max(...gradient.stops.map(s => s.offset)) + 0.1) : 0.5
    ...
    const handleRadiusChange = (value: number) => {
        updateGradient({
            ...gradient,
            radius: Math.max(0, Math.min(100, value)),
        })
    }
            ...
            <div className="flex flex-col gap-4 w-full">
                ...
                <div className="flex gap-1 w-fit h-fit">
                    ...
                    <div className="flex flex-col gap-2">
                        <Input title="CX" type="number" min={0} max={100} value={gradient.cx} onChange={value => handleCenterChange('cx', value)} />
                        <Input title="CY" type="number" min={0} max={100} value={gradient.cy} onChange={value => handleCenterChange('cy', value)} />
                        <Input title="R" type="number" min={0} max={100} value={gradient.radius} onChange={handleRadiusChange} />
                    </div>
                </div>
