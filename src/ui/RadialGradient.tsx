...
const RadialGradientPicker: React.FC<RadialGradientPickerProps> = ({
    value,
    onGradientChange,
    className,
}) => {
    ...
    const addStop = () => {
        const newOffset =
            gradient.stops.length > 0
                ? Math.min(
                      1,
                      Math.max(...gradient.stops.map(s => s.offset)) + 0.1
                  )
                : 0.5
        ...
        <div className={twMerge(`w-full ${className}`)}>
            <div className="flex flex-col gap-4 w-full">
                ...
                <div className="flex gap-1 w-fit h-fit">
                    <DirectionControls
                        directions={RADIAL_PRESET_DIRECTIONS}
                ...
                />
                <PresetGradients
                    gradients={PRESET_RADIAL_GRADIENTS}
