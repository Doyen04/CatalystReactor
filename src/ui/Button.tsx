...
const Button = forwardRef<HTMLButtonElement, ButtonProps>(
    ({ tool, className, group, children, active, ...props }, ref) => {
        ...
        const isActive = group.some(
            grouptool => currentTool.toolName === grouptool.toolName
        )
        ...
        const buttonStyles = useMemo(() => {
            const baseStyles =
                'w-fit h-fit p-1.5 rounded-lg border transition-colors duration-200'
            ...
            <div className="flex items-center gap-0.5 h-fit w-fit">
                <div className="relative group">
                    <button
                        ref={ref}
                        onClick={() => {
                            if (currentTool.toolName !== tool.toolName)
                                setTool(tool)
                        }}
