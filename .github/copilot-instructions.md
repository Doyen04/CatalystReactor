...
- Property editing loop:
    ...
    - `ShapeManager` runs property changes through the `CommandManager` and notifies `EngineStateStore` on every change; property sync to the store is throttled during drags (`throttle`, ~100ms).
