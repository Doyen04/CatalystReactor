...
- `src/engine/createEditor.ts` is the composition root: it builds one per-editor `EngineBus`, `DocumentModel`, `EngineStateStore(doc)`, `CommandManager`, `ServiceContainer`, and `SnapManager`. `src/component/Canvas.tsx` is the mount point:
    - Lazily boots CanvasKit wasm once via a module-level `ensureCanvasKit()` cached promise (`canvaskit-wasm/bin/canvaskit.wasm?url`; fonts via `CanvasKitResources.loadInterFont()` from `public/fonts` + remote families in `src/lib/core/fonts.json`, best-effort `Promise.allSettled`).
    - Then `editor.attach(canvas) -> new CanvasManager(canvas, deps)`; the only process-wide singleton left is `CanvasKitResources`.
- `CanvasManager` registers core services in the per-editor `ServiceContainer` in this order:
    - `paintManager` -> `shapeModifier` -> `commandManager` -> `shapeManager` -> `sceneManager` -> `inputManager` -> `renderer` -> `toolManager`
- Service resolution for tools/managers relies on this container setup (typed `ServiceRegistry`, per-editor `new ServiceContainer()`, `container.resolve(key)` warns + returns `null` if missing). If you add a new shared subsystem, register it here. Do not instantiate manager copies inside tools/components.
...
- Root scene is a `ContainerNode` with no shape (`new ContainerNode(null, null, container)`); child nodes hold visible scene content.
- Hit-testing:
...
- Current tools:
    ...
    - `KeyboardTool` (constructed by `ToolManager` as `(shapeManager, commandManager, doc)` with the injected per-editor `doc`): printable chars/Enter/arrows/Delete/Backspace route to the text-capable selected shape, arrows also nudge the selected shape, Ctrl+Z / Ctrl+Y drive `commandManager.undo()/redo()`, and Escape calls `shapeManager.cancelDrag()` while `doc.isTransactionOpen` to revert a mid-drag change.

## Shapes and Property Pipeline

- Shape creation goes through `ShapeFactory.createShape(type, pos, container, store, image?)` with `ShapeType` values; a `ShapeData` is registered in the per-editor `EngineStateStore` (source of truth `shapeDataMap`, keyed by `crypto.randomUUID()` id).
...
- Snapping: per-editor `SnapManager` (`new SnapManager()` in `createEditor()`) computes grid + shape-edge snap (`getSnapResult`, `snapDistance` 8px default). The grid size is no longer read from `useSceneStore`: `Canvas.tsx` pushes it via `CanvasManager.setGridSize` -> `ShapeManager.setGridSize`, which also forwards it into `snap.setConfiguration`; `ShapeManager.drag()` passes it to `getSnapResult` and patches the mouse event with snapped coords. `ShapeManager.drawSnapGuides()` renders indicators/guides (cached paint + dash are lazily created and per-`ShapeManager`).
...
- `EditorProvider` (`src/bridge/EditorProvider.tsx`, mounted inside `CanvasManagerProvider`): one provider mounts exactly one editor via `useState(() => createEditor())` and disposes it on unmount. `src/bridge/editor.ts` only re-exports the `Editor` type (`export type { Editor as EditorHandle } from '@/engine/createEditor'`) and `createEditor`. The editor handle exposes `doc`/`bus`/`store`/`run`/`undo`/`redo`/`canUndo`/`canRedo`/`isAttached`/`attach`/`detach`/`dispose`/`setTool`/`setGridSize` through `useEditor()`. The bridge hooks `useEntity(id)` / `useEntityThrottled(id)` (50ms) / `useDocumentRevision()` (100ms) subscribe to the bus's `document:changed` event and read the (in-place-mutated) `DocumentModel` record fresh — they intentionally do NOT snapshot `doc.get(id)` through `useSyncExternalStore` because the record identity never changes.
...
- Step 8 (React cut-over, duplicates deleted) landed: `properties:changed` was replaced by `document:changed: { ids: string[] }` in `src/lib/core/EngineEvents.ts`. `ShapeManager` emits it per-frame with the touched id (unthrottled); `CommandManager` emits it on `commit()`/`undo()`/`redo()` (with `requestRender()`), so undo/redo now drive the panel too. New bridge layer: `src/bridge/editor.ts` (`makeEditor(canvasManager)` -> `EditorHandle { doc, bus, run, undo, redo, canUndo, canRedo }`), `EditorProvider.tsx`, `useEditor.ts`, `useEntity.ts` and `useEntityThrottled.ts` (50ms) and `useDocumentRevision.ts` (100ms) — the entity hooks subscribe to filtered `document:changed` and re-read `doc.get(id)` fresh (they deliberately do NOT use `useSyncExternalStore`, whose `Object.is` snapshot check silently skips re-render because records are mutated in place). `PropertyBar` is now a controlled component: it reads `useEntityThrottled(selectedShapeId)` and writes `commands.run(new UpdateProperties(id, null, patch))`; `src/bridge/propertyPatch.ts` provides pure unit-tested patch builders. `LayersPanel` reads its hierarchy from `doc` (`childrenOf(rootId)`, `ancestorsOf`, `layoutConstraints.type`) and refreshes via `useDocumentRevision()`. Deleted: `useSceneStore.currentShapeProperties` (+ `setCurrentShapeProperties`/`clearProperties` and the `EngineStateStore.subscribe` side-effect in `sceneStore.tsx`), `ShapeManager.{throttledUpdate,updateProperty,updateSubProperty,updateStyle,updateBorderRadius,updateRadiusLock}` and its `notify()` calls, the `properties:changed` mapping in `engineStoreBridge.ts`, and `src/lib/types/engine.ts`. Tests: `src/lib/__tests__/propertyPatch.test.ts` (+24), `engineStoreBridge.test.ts` resized, CommandManager render-count updated — 21 files / 379 tests total. Manual UI smoke (panel↔canvas round-trip + undo) still deferred; per-editor instances landed in Step 9 (below).

- Step 9 (per-editor instances) landed: no module-level singletons remain except `CanvasKitResources`. `DependencyManager` now exports `class ServiceContainer` (`register`/`resolve`/`clear`) instead of a module-level `container`; `EngineStateStore` gained `constructor(doc)` (dropped `getInstance()`); `SnapManager` gained a public `constructor()` (dropped `getInstance()`). `createEditor()` is the full composition root: it builds one `{ bus, doc, store, commandManager, container, snap }` per editor and returns an `Editor` (`doc/bus/store/isAttached/attach/detach/dispose/setTool/setGridSize/run/undo/redo/canUndo/canRedo`); `PaintManager` + `ShapeModifier` are constructed only inside `attach()` (they require CanvasKit), keeping `createEditor()` headless. Every `Shape`/primitive/`ContainerNode`/`TextCursor`/`ShapeFactory`/`ShapeModifier`/managers now take the per-editor `ServiceContainer` (or `doc`/`store`/`snap`) explicitly — `Shape(data, container)`, `ContainerNode(shape, layoutConstraints, container)`, `ShapeFactory.createShapeFromData(data, container, image?)`, `ShapeManager(shapeModifier, bus, commandManager, doc, snap)` (no store import; `setGridSize` also feeds `snap.setConfiguration`), `SceneManager(...)`, `ToolManager(cnvs, inputManager, bus, container, doc)`, `CanvasManager(canvas, deps: CanvasDeps)`. `src/bridge/editor.ts` is now just a type re-export; `EditorProvider` mounts one editor via `useState(() => createEditor())` + dispose on unmount (`makeEditor` gone). `Canvas.tsx` is the mount point: a module-level `ensureCanvasKit()` lazy promise boots WASM once, then `editor.attach(canvas)` (guarded by a local `cancelled` flag + `editor.isAttached()` for StrictMode), pushes the current tool/gridSize right after attach, and wires the `[skia]` counter. Tests: `src/engine/__tests__/perEditorIsolation.test.ts` (+2, two editors fully independent), singleton tests rewritten over per-instance constructors, `headless.test.ts` green — 22 files / 381 tests total. Two-side-by-side editors smoke check still deferred.
